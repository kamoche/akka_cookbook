...
import com.kamoche.chapter7.SubscriptionManager._
...
object DistributedDataApplication {
  val actorSystem = ActorSystem("ClusterSystem")
  Cluster(actorSystem).registerOnMemberUp {
    val subscriptionManager =
      actorSystem.actorOf(SubscriptionManager.props)
    val subscription = Subscription(Random.nextInt(3),
